...
int rotation_point_x=0;
int rotation_point_y=0;
...
//to get vague information about the orientation of the line.
/*
0 stands for unset
1 stands for positive trend
-1 stands for negative trend
*/
int Xvariance = 0;
int Yvariance = 0;
...
double getEuclidDistance(int previous_point_x, int previous_point_y, int new_point_x, int new_point_y){
  return sqrt(pow((previous_point_x-new_point_x),2)+pow((previous_point_y-new_point_y),2));
}
...
int* applyTransformation(int point_x, int point_y){
  ...
  //multiply by PI/180 to convert my angle to RAD.
  ...
  point_transformation[0]=(int)round(rotatedX)+translation_x;
  point_transformation[1]=(int)round(rotatedY)+translation_y;
...
void computeTransformationLine(int startX_line, int startY_line, int endX_line, int endY_line){
  ...
  //using max...
  /*if(euclid_distance<max_length_compound_line){
    return;
  }
  max_length_compound_line=euclid_distance;*/
  //using euclid distance threshold
  if(euclid_distance<5){
    return;
  }
  std::cout<<"Euclid Distance: "<<euclid_distance<<std::endl;
  ...
  for (std::vector<double>::iterator it = euclid_distance_lines.begin() ; it != euclid_distance_lines.end(); ++it){
    if((*it+threshold_length_line>=euclid_distance)&& (*it-threshold_length_line<=euclid_distance)){
      ...
      //our rotation point is always the center of the image, because this is how I draw the scans.
      //NOT SURE!
      ...
      std::cout<<"Possible translation: ("<<translation_x<<","<<translation_y<<") and rotation: "<<rotation_angle<<" degree"<<std::endl;
    }
...
bool analyzeMissingRoomInformation(double angle, double distance, int center_point_x, int center_point_y){
  ...
  if(angle<=angle_previous+threshold_angle){
    ...
    //to get a vague sense about the orientation of the line.
    //project points to know their difference.
    /*int adjacentVal_previous = (int)(cos(angle_previous*PI/180.0)*distance_previous);
    int oppositeVal_previous = (int)(sin(angle_previous*PI/180.0)*distance_previous);
    int adjacentVal_new = (int)(cos(angle*PI/180.0)*distance);
    int oppositeVal_new = (int)(sin(angle*PI/180.0)*distance);
    //compute trend
    int trendX = (center_point_x+adjacentVal_previous)-(center_point_x+adjacentVal_new);
    int trendY = (center_point_y+oppositeVal_previous)-(center_point_y+oppositeVal_new);

    //set them if they have not been set
